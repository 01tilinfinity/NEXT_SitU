...
class ChangePasswordForm(PasswordVerificationMixin, UserForm):
    oldpassword = forms.CharField(
        label=_("현재 비밀번호"),
        widget=forms.PasswordInput(attrs={'autocomplete': 'current-password', 'class': 'form-control', 'placeholder': _("현재 비밀번호")})
    )
    password1 = forms.CharField(
        label=_("새로운 비밀번호"),
        widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': _("새로운 비밀번호")})
    )
    password2 = forms.CharField(
        label=_("새로운 비밀번호 (확인)"),
        widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': _("새로운 비밀번호 (확인)")})
    )
    ...
    def clean_oldpassword(self):
        if not self.user.check_password(self.cleaned_data.get("oldpassword")):
            raise forms.ValidationError(_("현재 비밀번호가 올바르지 않습니다."))
