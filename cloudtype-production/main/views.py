...
def ajax_search(request):
    ...
    if query:
        cafes = cafes.filter(Q(cafe_name__icontains=query))
...
def seat_overview(request, cafe_id):
    cafe = get_object_or_404(Cafe, id=cafe_id)
    seats = Seat.objects.filter(cafe_id=cafe.id)
    reservations = Reservation.objects.filter(seat__cafe_id=cafe.id)
    return render(request, 'reservations/seat_overview.html', {
        'cafe': cafe,
        'seats': seats,
